...
import Admin.Analytics.AnalyticsReport;
import Admin.Analytics.QuantityAnalyticsService;
...
public class AdministradorPantalla extends JFrame {
    ...
    private void accionBoton(String nombreBoton) {
        switch (nombreBoton) {
            ...
            case "Generar Reportes":
                SwingUtilities.invokeLater(() -> {
                    try {
                        QuantityAnalyticsService service = new QuantityAnalyticsService(this.fb);

                        AnalyticsReport reporte = service.generateReport();
    ...
    private void mostrarReporte(AnalyticsReport reporte) {
        JTextArea textArea = new JTextArea(20, 40);
        textArea.setEditable(false);
        textArea.setFont(new Font("Monospaced", Font.PLAIN, 14));

        StringBuilder sb = new StringBuilder();
        sb.append(" --- REPORTE DE ANALÍTICAS DE CANTIDAD ---\n\n");
        sb.append("TOTALES GLOBALES\n");
        sb.append("===========================================\n");
        sb.append(String.format("Total de Usuarios:   %d\n", reporte.totalUsuarios));
        sb.append(String.format("Total de Cursos:     %d\n", reporte.totalCursos));
        sb.append(String.format("Total de Asignaturas: %d\n", reporte.totalAsignaturas));
        sb.append(String.format("Total de Clases:     %d\n", reporte.totalClases));

        sb.append("\nUSUARIOS POR ROL\n");
        sb.append("===========================================\n");
        if (reporte.conteoPorRol != null) {
            reporte.conteoPorRol.forEach((rol, cant) ->
        ...
        sb.append("===========================================\n");
        if (reporte.clasesPorAsignatura != null) {
            reporte.clasesPorAsignatura.forEach((asig, cant) ->
                    sb.append(String.format("  %-20s: %d\n", asig, cant))
            );
        }
        ...
        sb.append("\nCLASES POR DOCENTE\n");
        sb.append("===========================================\n");
        if (reporte.clasesPorDocente != null) {
            reporte.clasesPorDocente.forEach((docente, cant) ->
                    sb.append(String.format("  %-20s: %d\n", docente, cant))
            );
        }
        ...
        JDialog dialog = new JDialog(this, "Reporte de Analíticas", true);
